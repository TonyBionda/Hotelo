...
public class ProduitAddController implements Initializable {
    ...
    public void valider() {
        try {
            ...
            String prixtext = input_prix.getText();
            for (int i = 0; hoteltext.charAt(i) != ' '; i++) {
                hotel_id += hoteltext.charAt(i);
            }
            Integer.parseInt(hotel_id);
            if (nom.length() == 0 || hoteltext.length() == 0 || prixtext.length() == 0) {
                System.out.println("Faut mettre toutes les valeurs");
            } else {
                String query = " insert into produit (nom, prix, estActif, hotel_id)"
                        + " values (?, ?, ?, ?)";
                PreparedStatement preparedStmt = connection.prepareStatement(query);
                preparedStmt.setString(1, nom);
                preparedStmt.setFloat(2, Float.valueOf(prixtext));
                preparedStmt.setBoolean(3, true);
                preparedStmt.setString(4, hotel_id);
